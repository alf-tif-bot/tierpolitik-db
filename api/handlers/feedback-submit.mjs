...
const autoImproveSummary = ({ title = '', body = '', note = '' }) => {
  ...
  if (titleClean) {
    return `${titleClean}. Fokus: tierpolitische Einordnung und verstÃ¤ndliche Kurzbeschreibung.`
  }
  if (bodyClean) return bodyClean.slice(0, 220)
  return 'Kurzbeschreibung automatisch Ã¼berarbeitet.'
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    const summary = `[${category}] ${message || 'Kein Zusatztext'} Â· GeschÃ¤ft: ${businessNo}`
        ...
        for (const row of motions.rows) {
          ...
          if (isIrrelevant) {
            await client.query(
              `update motions
               set status = 'queued',
                   review_reason = coalesce(nullif(review_reason, ''), 'User feedback') || ' Â· user-feedback=irrelevant',
                   updated_at = now()
               where id = $1`,
              [motionId],
            )
          ...
          if (isDescriptionImprove) {
            ...
            await client.query(
              `update motions
               set review_reason = coalesce(nullif(review_reason, ''), 'User feedback') || ' Â· user-feedback=description-improved',
                   updated_at = now()
               where id = $1`,
              [motionId],
            )
