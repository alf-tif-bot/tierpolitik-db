...
const inferType = (title = '', sourceId = '') => {
  ...
  if (text.includes('petition') || text.includes('pÃ©tition') || text.includes('petizione')) return 'Petition'
...
const extractStance = (reason = '', title = '', summary = '', body = '') => {
  ...
  const m = String(reason).match(/stance=([^Â·]+)/)
...
const buildStatusTimeline = (rows = [], fallbackDate = '') => {
  const items = (Array.isArray(rows) ? rows : [])
    .map((row) => {
      ...
      return { datum: date, status, bemerkung: 'Stand gemÃ¤ss Parlamentsdaten' }
    })
  ...
  if (!deduped.length) {
    return [{ datum: fallbackDate || new Date().toISOString().slice(0, 10), status: 'In Beratung', bemerkung: 'Stand gemÃ¤ss Parlamentsdaten' }]
  }
...
const typeLabels = {
  ...
  Petition: { de: 'Petition', fr: 'PÃ©tition', it: 'Petizione', en: 'Petition' },
...
const formatBusinessNumber = (title = '', externalId = '', summary = '', body = '') => {
  const bodyMatch = String(body || '').match(/GeschÃ¤ftsnummer:\s*([A-Za-z0-9.\-]+)/i)
  if (bodyMatch?.[1]) return bodyMatch[1]
  const summaryMatch = String(summary || '').match(/Â·\s*([0-9]{4}\.[A-Z]{2}\.[0-9]{4}|\d{2}\.\d{4})\s*Â·/)
...
const fallbackPersonByLang = {
  de: { name: 'GemÃ¤ss Curia Vista', rolle: '', partei: '' },
...
const SUBMITTER_OVERRIDES = {
  '25.4380': { name: 'Mathilde Crevoisier Crelier', rolle: 'StÃ¤nderat', partei: 'SP' },
  '24.3277': { name: 'Lorenz Hess', rolle: 'Nationalrat', partei: 'Die Mitte' },
  '25.404': { name: 'Kommission fÃ¼r Wissenschaft, Bildung und Kultur Nationalrat', rolle: 'Kommission', partei: '' },
  '21.3002': { name: 'Kommission fÃ¼r Umwelt, Raumplanung und Energie StÃ¤nderat', rolle: 'Kommission', partei: '' },
  '22.3299': { name: 'Schneider Meret', rolle: 'Nationalrat', partei: 'GrÃ¼ne Partei der Schweiz' },
...
const SUMMARY_OVERRIDES = {
  '21.3002': 'Die Motion verlangt, den Handlungsspielraum im Jagdgesetz per Verordnung auszuschÃ¶pfen, um die Koexistenz zwischen Menschen, Grossraubtieren und Nutztieren zu regeln (u. a. Regulierung und Herdenschutz).',
  '25.4809': 'Der Vorstoss verlangt konkrete Massnahmen gegen Tierqual bei der GeflÃ¼gelschlachtung und eine konsequent tierschutzkonforme Praxis.',
  ...
  '24.331': 'Die Standesinitiative des Kantons Jura verlangt finanzielle UnterstÃ¼tzung fÃ¼r Imkerinnen und Imker bei geoklimatischen Ausnahmebedingungen, insbesondere fÃ¼r notwendige ZuckerfÃ¼tterung zur Sicherung des Ãœberlebens von HonigbienenvÃ¶lkern.',
}
...
const inferSubmitter = (lang, title = '', summary = '', body = '') => {
  const text = `${title} ${summary} ${body}`.toLowerCase()
  if (text.includes('blv') || text.includes('lebensmittelsicherheit') || text.includes('veterinÃ¤rwesen')) {
    return { name: 'BLV', rolle: 'Regierung', partei: 'Bundesverwaltung' }
  }
  if (text.includes('eingereicht von bundesrat') || text.includes('message du conseil fÃ©dÃ©ral') || text.includes('messaggio del consiglio federale')) {
...
const normalizeDisplayTitle = (row, title = '') => {
  ...
  if (isBern) t = t.replace(/^Bern\s*[Â·:-]\s*/i, '')
  return t
}
...
const formatThemeLabel = (value = '') => {
  ...
  if (/^geflÃ¼gel$/i.test(s) || /^gefluegel$/i.test(s)) return 'MasthÃ¼hner'
...
const municipalThemesFromTitle = (title = '') => {
  ...
  const out = []
  if (t.includes('feuerwerk') || t.includes('lÃ¤rm') || t.includes('laerm')) out.push('Feuerwerk')
  if (t.includes('tierpark')) out.push('Zoo')
  if (t.includes('biodivers')) out.push('BiodiversitÃ¤t')
...
const isWeakSummarySentence = (text = '') => {
  ...
  return s.includes('stellungnahme zum vorstoss liegt vor')
    ...
    || s.includes('Ã¼berwiesen an den bundesrat')
    || s.includes('ueberwiesen an den bundesrat')
    || s.includes('|')
    || /^parlamentsgesch(Ã¤|a)ft\s+/i.test(s)
}
...
const summarizeVorstoss = ({ title = '', summary = '', body = '', status = '', sourceId = '' }) => {
  ...
  if (low.includes('chlorhÃ¼hner') || low.includes('chlorhuehner') || (low.includes('geflÃ¼gel') && low.includes('importverbot'))) {
    sentences.push('Der Vorstoss verlangt ein klares Importverbot fÃ¼r chemisch behandeltes GeflÃ¼gelfleisch ("ChlorhÃ¼hner") und die Verankerung im Gesetz.')
    ...
    sentences.push('Dieser Vorstoss betrifft die Stopfleber-Thematik (Foie gras) und die politische Umsetzung eines indirekten Gegenentwurfs mit stufenweisen ImportbeschrÃ¤nkungen.')
  ...
  } else if (low.includes('tierversuch') || low.includes('3r') || low.includes('expÃ©rimentation animale')) {
    sentences.push('Dieser Vorstoss behandelt Alternativen zu Tierversuchen (3R) und die Frage, wie Forschung gezielt in tierfreie bzw. tierÃ¤rmere Methoden gelenkt werden kann.')
    ...
    sentences.push('FÃ¼r die Einordnung ist zentral, ob die vorgeschlagenen Massnahmen den Schutzstatus stÃ¤rken oder Eingriffe ausweiten.')
  }
  ...
  try {
    ...
    const mapped = dedupedRows.map((r, index) => {
      ...
      const summaryText = normalizedSummary.length >= 10
        ? normalizedSummary
        : `KurzÃ¼berblick: ${displayTitle || `Vorstoss ${index + 1}`} (${statusLabel}).`
