...
- `npm run crawler:collect:v2`: affair-zentrierte Bundesdaten (DE/FR/IT zusammengeführt) + kantonales Registry-Scaffold (parallelisiert; per `CRAWLER_COLLECT_CONCURRENCY` und `CRAWLER_COLLECT_TIMEOUT_MS` steuerbar)
- `npm run sources:build-cantonal-registry`: erzeugt `data/cantonal-source-registry.json` (alle 26 Kantone, Zielhistorie ab 2020) inkl. URL-Probe (HTTP-Status, Final-URL, Plattform-Hinweis, Readiness), Adapter-Verteilung, offenen Kantonen und `sinceYear`-Verstössen
