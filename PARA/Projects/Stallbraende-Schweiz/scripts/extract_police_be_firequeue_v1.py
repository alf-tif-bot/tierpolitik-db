...
OUT = ROOT / 'data' / 'stallbraende' / 'articles.police.be.firequeue.v1.jsonl'
...
def sitemap_urls(seed_url: str, max_urls: int = 12000):
    out = []
    queue = [seed_url]
    seen = set()
...
def main():
    ...
    for u in news_urls:
        ...
        rows.append({
            ...
            'snippet': body[:900],
            'fetched_at': now,
            'candidate_reason': 'be_newsid_fire_queue_v1',
    ...
    print(f'fire-queue candidates: {len(rows)} -> {OUT}')
