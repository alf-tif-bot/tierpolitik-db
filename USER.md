# USER.md - About Your Human

_Learn about the person you're helping. Update this as you go._

- **Name:** Tobi
- **What to call them:** Tobi
- **Pronouns:**
- **Timezone:** Europe/Zurich
- **Notes:** Wants concise, practical support. Strongly dislikes people-pleasing tone, hallucinations, not admitting mistakes, and micromanaging.

## Context

### 3-month direction
Build a digital “Animal Liberation Front” powered by specialized agents.

### Core outcomes
- Create strong writing output: media releases, newsletters, book project drafts
- Build and maintain systems: Monitor, Cockpit, and related tooling
- Publish regularly on TIF website
- Crawl/research for new animal-rights ideas and opportunities
- Support fundraising and supporter operations

### Tasks to proactively offload
- Writing/text production
- Publishing workflows
- Data processing
- Research

### Priority domains
1. Tier im Fokus (TIF)
2. Personal health
3. Finance (Bitcoin, Altcoins, Stocks)

### Response style preference
Default short answers unless depth is explicitly requested.

---

The more you know, the better you can help. But remember — you're learning about a person, not building a dossier. Respect the difference.


### Operating mode
- Proactive check-ins: yes (for important opportunities/risks)
- Language: prioritize best-quality output language by task (DE/EN as needed)

### Autonomy boundaries
- Ask first: emails, spending money, trading execution
- Autonomous: coding, research, crawling, text drafts based on templates/briefs

### Finance support
- Wants concrete buy/sell proposals (with clear rationale and risk framing)
