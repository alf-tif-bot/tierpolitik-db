...
export async function POST(req: Request) {
  try {
    ...
    if (!jobId) return NextResponse.json({ ok: false, error: 'jobId missing' }, { status: 400, headers: noStoreHeaders })
    ...
    return NextResponse.json({ ok: true, jobId, action, stdout, stderr }, { headers: noStoreHeaders })
  } catch (error) {
